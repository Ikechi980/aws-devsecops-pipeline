...
QUEUE_NAME="resources-change-logger-events"
LAMBDA_URL="$CHANGE_LOGGER_INVOKE_URL_LOCAL"
...
PY
    ...
    status=$(curl -s -o /tmp/resources-change-logger.bridge.response \
        -w "%{http_code}" \
        -X POST "$LAMBDA_URL" \
    ...
    if [[ "$status" == "200" ]]; then
        receipts=$(python3 - <<'PY' "$resp" "/tmp/resources-change-logger.bridge.response"
import json
import sys
