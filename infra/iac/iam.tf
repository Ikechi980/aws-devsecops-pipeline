...
resource "aws_iam_role_policy" "lambda_exec_ssm" {
    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:ssm:${var.region}:${data.aws_caller_identity.current.account_id}:parameter/sentrics-core/${var.environment}/database-url"
      },
      {
        Effect   = "Allow"
        Action   = ["kms:Decrypt"]
